...
source ./ci/gryphonmoon/matrix.sh
...
cd build-ci/gryphonmooncore-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_gryphonmoon.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ${TEST_RUNNER_ENV} ./src/test/test_gryphonmoon --catch_system_errors=no -l test_suite
  else
