...
* [ ] Update manpages, see [gen-manpages.sh](https://github.com/gogryphon/gryphoonmoon/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
git clone https://github.com/gryphonmoon/gryphonmoon.git
```

### Gryphonmoon Core maintainers/release engineers, suggestion for writing release notes
...
Checkout the Gryphonmoon Core version you'd like to build:
...
_Note: this step can be skipped if [our CI](https://github.com/gogryphon/gryphoonmoon/blob/master/ci/test/00_setup_env.sh#L64) still uses bitcoin's SDK package (see SDK_URL)_
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../gryphonmoon/contrib/builder-keys/README.md`.
...
* Transfer `gryphonmooncore-osx-unsigned.tar.gz` to macOS for signing
* Extract and sign:

    ```sh
    tar xf gryphonmooncore-osx-unsigned.tar.gz
...
* Extract and sign:
    ...
    ```sh
    tar xf gryphonmooncore-win-unsigned.tar.gz
...
```sh
pushd ~/gryphonmooncore-detached-sigs
...
- Detached signatures will then be committed to the [gryphonmoon-detached-sigs](https://github.com/gogryphon/gryphoonmoon-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
* [ ] Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to GitHub as GitHub draft release.
    1. The contents of each `./gryphonmoon/guix-build-${VERSION}/output/${HOST}/` directory, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the gryphonmoon.org server*.

       ```sh
       find guix-build-${VERSION}/output/ -maxdepth 2 -type f -not -name "SHA256SUMS.part" -and -not -name "*debug*" -exec scp {} user@gryphonmoon.org:/var/www/bin/gryphonmoon-core-${VERSION} \;
       ```
...
* [ ] Update the gryphonmoon.org download links
* [ ] Ensure that docker hub images are up to date

### Announce the release:
* [ ] Release on Gryphonmoon forum: https://www.gryphonmoon.org/forum/topic/official-announcements.54/ (necessary so we have a permalink to use on twitter, reddit, etc.)
...
* [ ] Update Core and User docs (docs.gryphonmoon.org)
* [ ] Test Docker build runs without error in Gryphonmoonmate
...
```sh
xcrun notarytool submit gryphonmooncore-{version}-{x86_64, arm64}-apple-darwin.dmg --keychain-profile "AC_PASSWORD" --wait
```
...
After successfully notarizing the .dmg file, extract `Gryphonmoon-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Gryphonmoon-Qt.app
```

Replace `Gryphonmoon-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
