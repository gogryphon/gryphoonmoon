...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 GRYPHONMOON from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 GRYPHONMOON in block rewards plus fees, but
        ...
        # Send 100 GRYPHONMOON with subtract fee from amount
        ...
        # Sendmany 100 GRYPHONMOON with subtract fee from amount
        ...
        # Sendmany with explicit fee (GRYPHONMOON/kB)
        ...
        # This will raise an exception for attempting to get the private key of an invalid Gryphonmoon address
        assert_raises_rpc_error(-5, "Invalid Gryphonmoon address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid Gryphonmoon address
        assert_raises_rpc_error(-5, "Invalid Gryphonmoon address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid Gryphonmoon address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit gryphonmoon/kb fee
        self.log.info("test explicit fee (sendtoaddress as gryphonmoon/kb)")
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        total_txs = len(self.nodes[0].listtransactions("*", 118209))
        ...
        assert_equal(total_txs, len(self.nodes[0].listtransactions("*", 118209)))
